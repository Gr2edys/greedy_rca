...
def parse_navi_xml(navi_path):
    """从 navi.xml 提取所有 MML Document 文件路径和命令名映射"""
    with open(navi_path, 'r', encoding='utf-8') as f:
        content = f.read()

    doc_paths = set()
    path_cmd_map = {}  # path -> cmd_name (从 navi 的 topic txt 中提取)
    ...
    # 从 navi 中提取命令名：匹配 topic txt="...（CMD NAME）" url="MML/Document/xxx.html"
    for m in re.finditer(
        ...
        path_cmd_map[path] = cmd

    return sorted(doc_paths), path_cmd_map
...
def main(resources_dir, output_path):
    ...
    print("解析 navi.xml ...")
    doc_paths, path_cmd_map = parse_navi_xml(navi_path)
    ...
    for i, rel_path in enumerate(doc_paths):
        ...
        if not cmd_name:
            # 回退：从 navi.xml 中获取命令名
            cmd_name = path_cmd_map.get(rel_path, '')
        if not cmd_name:
            # 最终回退：从文件名提取
